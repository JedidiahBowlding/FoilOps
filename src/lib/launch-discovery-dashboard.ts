    ...
    return renderFuturisticPage({
      ...
      contentHtml: `
        ...
        <section class="section card base-swap-workbench">
          ...
          <div class="research-controls"><button class="fx-button secondary" id="base-swap-quote" type="button">Get Safe Quote</button><button class="fx-button secondary" id="base-swap-watch" type="button">Start Launch Watch</button><button class="fx-button secondary" id="base-swap-stop" type="button" disabled>Stop Watch</button><button class="fx-button" id="base-swap-confirm" type="button" disabled>Confirm Live Swap</button></div>
          <div id="base-swap-status" class="notice">A quote does not execute a transaction.</div><div id="base-swap-quote-result"></div>
        </section>
      ...
      scriptHtml: `
        <script>
          ...
          let baseSwapConfirmation=null,baseSwapWatchTimer=null,baseSwapWatchBusy=false,baseSwapWatchAttempts=0;
          ...
          async function requestBaseQuote(watching=false){const button=document.getElementById('base-swap-quote'),status=document.getElementById('base-swap-status'),out=document.getElementById('base-swap-quote-result'),confirm=document.getElementById('base-swap-confirm');if(baseSwapWatchBusy)return false;baseSwapWatchBusy=true;baseSwapConfirmation=null;confirm.disabled=true;if(!watching)button.disabled=true;status.textContent=watching?'Watching for a valid route… attempt '+(++baseSwapWatchAttempts):'Checking wallet balance, token risk, liquidity and live route…';try{const response=await fetch('/api/base-swap/quote',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sellToken:document.getElementById('swap-sell-token').value,buyToken:document.getElementById('swap-buy-token').value,amount:document.getElementById('swap-amount').value,slippageBps:Number(document.getElementById('swap-slippage').value)})});const q=await response.json();if(!response.ok)throw new Error(q.message||'Quote failed');baseSwapConfirmation=q.confirmationId;confirm.disabled=false;stopBaseSwapWatch();status.textContent='Route is live. Quote expires '+new Date(q.expiresAt).toLocaleTimeString()+'. Review and press Confirm Live Swap.';out.innerHTML='<div class="research-grid"><div class="research-panel"><span>You sell</span><strong>'+esc(q.sellAmount)+' '+esc(q.sellSymbol)+'</strong></div><div class="research-panel"><span>Expected</span><strong>'+esc(q.expectedBuyAmount)+' '+esc(q.buySymbol)+'</strong></div><div class="research-panel"><span>Minimum</span><strong>'+esc(q.minimumBuyAmount)+' '+esc(q.buySymbol)+'</strong></div><div class="research-panel"><span>Token risk</span><strong>'+esc(q.researchRisk??'—')+'</strong></div></div><div class="notice quote-warning">A valid route was found. No transaction occurs until you press Confirm Live Swap.</div>';document.title='ROUTE LIVE · Confirm Base Swap';return true;}catch(error){status.textContent=watching?'Still watching · '+esc(error.message||'No route yet'):(error.message||'Quote failed');if(!watching)out.innerHTML='';return false;}finally{baseSwapWatchBusy=false;button.disabled=false}}
          function stopBaseSwapWatch(){if(baseSwapWatchTimer){clearInterval(baseSwapWatchTimer);baseSwapWatchTimer=null}document.getElementById('base-swap-watch').disabled=false;document.getElementById('base-swap-stop').disabled=true}
          document.getElementById('base-swap-quote').addEventListener('click',()=>requestBaseQuote(false));
          document.getElementById('base-swap-watch').addEventListener('click',async()=>{const mint=document.getElementById('swap-buy-token').value.trim(),amount=document.getElementById('swap-amount').value.trim();if(!mint||!amount){document.getElementById('base-swap-status').textContent='Enter the Base contract and amount before starting the watch.';return}stopBaseSwapWatch();baseSwapWatchAttempts=0;document.getElementById('base-swap-watch').disabled=true;document.getElementById('base-swap-stop').disabled=false;await requestBaseQuote(true);if(!baseSwapConfirmation)baseSwapWatchTimer=setInterval(()=>requestBaseQuote(true),15000)});
          document.getElementById('base-swap-stop').addEventListener('click',()=>{stopBaseSwapWatch();document.getElementById('base-swap-status').textContent='Launch watch stopped. No transaction was submitted.'});
