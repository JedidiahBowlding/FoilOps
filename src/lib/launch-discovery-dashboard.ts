    ...
    return renderFuturisticPage({
      ...
      contentHtml: `
        ...
        <section class="section card base-swap-workbench">
          ...
          <div class="research-controls"><button class="fx-button secondary" id="base-swap-quote" type="button">Get Safe Quote</button><button class="fx-button" id="base-swap-confirm" type="button" disabled>Confirm Live Swap</button></div>
      ...
      scriptHtml: `
        <script>
          ...
          document.getElementById('base-swap-quote').addEventListener('click',async()=>{const button=document.getElementById('base-swap-quote'),status=document.getElementById('base-swap-status'),out=document.getElementById('base-swap-quote-result'),confirm=document.getElementById('base-swap-confirm');baseSwapConfirmation=null;confirm.disabled=true;button.disabled=true;status.textContent='Checking wallet balance, token risk, liquidity and live route…';try{const response=await fetch('/api/base-swap/quote',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({sellToken:document.getElementById('swap-sell-token').value,buyToken:document.getElementById('swap-buy-token').value,amount:document.getElementById('swap-amount').value,slippageBps:Number(document.getElementById('swap-slippage').value)})});const q=await response.json();if(!response.ok)throw new Error(q.message||'Quote failed');baseSwapConfirmation=q.confirmationId;confirm.disabled=false;status.textContent='Quote ready. It expires '+new Date(q.expiresAt).toLocaleTimeString()+'. Review before confirming.';out.innerHTML='<div class="research-grid"><div class="research-panel"><span>You sell</span><strong>'+esc(q.sellAmount)+' '+esc(q.sellSymbol)+'</strong></div><div class="research-panel"><span>Expected</span><strong>'+esc(q.expectedBuyAmount)+' '+esc(q.buySymbol)+'</strong></div><div class="research-panel"><span>Minimum</span><strong>'+esc(q.minimumBuyAmount)+' '+esc(q.buySymbol)+'</strong></div><div class="research-panel"><span>Token risk</span><strong>'+esc(q.researchRisk??'—')+'</strong></div></div><div class="notice quote-warning">This will spend real funds from the dedicated Base execution wallet. Quotes can move before confirmation.</div>';}catch(error){status.textContent=error.message||'Quote failed';out.innerHTML='';}finally{button.disabled=false}});
