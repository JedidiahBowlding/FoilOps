    ...
    return renderFuturisticPage({
      ...
      scriptHtml: `
        <script>
          ...
          document.getElementById('run-agent-contract').addEventListener('click',async()=>{const address=document.getElementById('research-mint').value.trim(),chain=document.getElementById('research-chain').value,button=document.getElementById('run-agent-contract'),status=document.getElementById('research-status'),out=document.getElementById('research-result');if(!address)return void(status.textContent='Paste a contract or mint first.');button.disabled=true;status.textContent='Collecting evidence, then running four independent analysts and risk synthesis…';try{const response=await fetch('/api/trading-agents/contract',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({address,chain})}),r=await response.json();if(!response.ok)throw new Error(r.message||'AI research failed');status.textContent='AI research team completed for exact '+chain+' address.';out.innerHTML='<h3>Risk-manager dossier</h3><div class="notice" style="white-space:pre-wrap">'+esc(r.decision)+'</div><details style="margin-top:12px"><summary>Independent analyst reports</summary><pre style="white-space:pre-wrap;overflow-wrap:anywhere">'+esc(JSON.stringify(r.reports,null,2))+'</pre></details>';}catch(error){status.textContent=error.message||'AI research failed';}finally{button.disabled=false}});
