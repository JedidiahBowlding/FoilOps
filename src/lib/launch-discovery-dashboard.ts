        ...
        <script>
          ...
          async function loadAutoBuy(){
            try{
              ...
              document.querySelectorAll('[data-ab-arm]').forEach(b=>b.addEventListener('click',async()=>{if(!await window.foilopsConfirm({title:'ARM reviewed order',message:'Authorize unattended execution for this exact contract within the displayed spend, liquidity, price impact, slippage, gas, route and deadline limits.',confirmLabel:'ARM order',danger:true}))return;b.disabled=true;b.textContent='Arming…';await autoBuyAction('/api/liquidity-auto-buy/orders/'+encodeURIComponent(b.dataset.abArm)+'/arm','Order ARMED. Monitoring exact-address liquidity now.')}));
              document.querySelectorAll('[data-ab-cancel]').forEach(b=>b.addEventListener('click',async()=>{if(!await window.foilopsConfirm({title:'Emergency cancel',message:'Cancel this order immediately? A cancelled order cannot execute.',confirmLabel:'Cancel order',danger:true}))return;b.disabled=true;b.textContent='Cancelling…';await autoBuyAction('/api/liquidity-auto-buy/orders/'+encodeURIComponent(b.dataset.abCancel)+'/cancel','Order cancelled.')}));
          ...
          document.getElementById('ab-create').addEventListener('click',async()=>{
            ...
            if(!await window.foilopsConfirm({title:'Create reviewed draft',message:summary+'\\n\\nThis creates a DRAFT only. It will not arm or execute the order.',confirmLabel:'Create draft'}))return;
          ...
          document.getElementById('ab-master').addEventListener('change',async(event)=>{const enabled=event.currentTarget.checked;if(enabled&&!await window.foilopsConfirm({title:'Enable Master AUTO BUY',message:'Only individually reviewed and ARMED orders can execute. All order-level limits remain enforced.',confirmLabel:'Enable master',danger:true})){event.currentTarget.checked=false;return}if(!enabled&&!await window.foilopsConfirm({title:'Disable Master AUTO BUY',message:'Monitoring and execution will stop immediately. Armed orders remain saved.',confirmLabel:'Disable master'})){event.currentTarget.checked=true;return}const response=await fetch('/api/liquidity-auto-buy/master',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({enabled})}),body=await response.json();document.getElementById('ab-status').textContent=response.ok?(enabled?'Master AUTO BUY is ON. Only explicitly ARMED orders are eligible.':'Master AUTO BUY is OFF. Monitoring and execution are stopped.'):(body.message||'Could not change master switch');await loadAutoBuy()});
