...
export function registerLiquidityAutoBuyRoutes(app:Express,auth:RequestHandler,expensive:RequestHandler,service:LiquidityAutoBuyService){
  app.get('/api/liquidity-auto-buy/status',auth,async(_req,res)=>res.json(await service.status()))
  app.post('/api/liquidity-auto-buy/master',auth,async(req,res)=>{try{if(typeof req.body?.enabled!=='boolean')throw new Error('enabled must be boolean');res.json(await service.setMasterEnabled(req.body.enabled))}catch(error){res.status(400).json({message:error instanceof Error?error.message:'Could not update master switch'})}})
